...
public class PayRecord {
  ...
  // complete the code to compute the gross pay for the employee based on the employee status
    ...
    if(this.employee.getEmpStatus() == Status.FullTime){
      return numMonths*montlyIncome;
    }

    if(this.employee.getEmpStatus() == Status.Hourly){
      double otGross = (payHours>REG_HOURS)?(payHours-REG_HOURS)*(payRate*OT_RATE):0;
      return Math.min(REG_HOURS, payHours)*payRate + otGross;
    }		
    return 0;
	}
  ...
  public double netPay(){
    return this.grossPay() - this.payTax.compIncomeTax(this.grossPay());
  }
